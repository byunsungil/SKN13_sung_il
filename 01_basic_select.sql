...
select distinct job  -- 중복 x, 한 컬럼에 대해서 확인
from emp;
...
select emp_id as 직원ID,  -- "직원 ID" 띄어 쓰기 ""로 묶어줌
		  emp_name as 직원이름,  -- "직원 이름"
          hire_date as 입사일, 
          salary as 급여, 
          dept_name as 소속부서
from emp;
...
-- emp_name, salary를 조회, salary앞 '$'를 붙여 출력
select emp_name, 
...
from emp
where salary > 5000
order by salary desc;  # order by 3 desc; , 3 : select 절 컬럼 순서
