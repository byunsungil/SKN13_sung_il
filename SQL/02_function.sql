...
select emp_id, 
		  emp_name, 
          salary, 
          ceil(salary * 1.15) as 'SAL_RAISE', # 소수점 이하 올림
          floor(salary * 1.15) as 'SAL_RAISE2' # 소수점 이하 버림
from emp;
...
select emp_id, 
		  emp_name, 
          comm_pct, 
          round(comm_pct * 1.08, 2) as 'comm_raise' # 소숫점 이하 2자리에서 반올림
from emp
...
select hire_date,
		  subdate(hire_date, interval 10 quarter),
          adddate(hire_date, interval 10 month),
          adddate(hire_date, interval 10 week);
...
-- comm_pct가 없으면 "커미션없음"을 있으면 "커미션있음"을 반환
 select if(comm_pct is null, '커미션없음', '커미션있음')
 from emp;
...
from emp
where job in ('AD_PRES', 'FI_ACCOUNT', 'PU_CLERK', 'IT_FROG');
