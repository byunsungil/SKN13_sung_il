...
select count(dept_name) -- 부서가 있는 직원수
from emp;

select count(distinct dept_name) -- 부서가 몇개
from emp;

select count(distinct ifnull (dept_name, 'a')) -- 결측치 포함 개수
from emp;
...
-- 업무(job)별 급여의 총합계, 평균, 최소값, 최대값, 표준편차, 분산을 조회
select job, sum(salary), avg(salary), min(salary), max(salary), stddev(salary), variance(salary)
from emp
group by job;
...
select  year(hire_date), job, avg(salary)
from emp
